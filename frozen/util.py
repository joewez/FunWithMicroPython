...
def ls(path='/', filter="", recurse=False, output=''):
    ...
    if recurse:
        for f in file_list:
            if f[1] == 16384:
                ls(merge(path, f[0]), filter, recurse, output)
...
def cp(src, dest):
    ...
    with open(src, "rb") as f1:
        with open(dest, "wb") as f2:
            count = f1.readinto(buffer)
            while count > 0:
                f2.write(buffer, count)
                count = f1.readinto(buffer)
